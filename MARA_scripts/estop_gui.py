...
# ---- Defaults you can change once and forget ----
DEFAULT_HOST = os.getenv("SPOT_HOST", "192.168.80.3")
DEFAULT_USER = os.getenv("SPOT_USER", "admin")
DEFAULT_PASS = os.getenv("SPOT_PASSWORD", None)

# Optional creds file with two lines: username on line 1, password on line 2
DEFAULT_CRED_PATH = Path(r"C:\Users\soere\OneDrive\Desktop\spot_creds.txt")
...
def resolve_connection_params(options):
    """
    Merge CLI options with env vars and a local creds file.
    Precedence: CLI > env vars > creds file > defaults.
    Returns: (host, user, password)
    """
    host = options.hostname or os.getenv("SPOT_HOST") or DEFAULT_HOST
    user = options.username or os.getenv("SPOT_USER") or DEFAULT_USER
    pwd  = options.password or os.getenv("SPOT_PASSWORD") or DEFAULT_PASS

    # Fallback to creds file if password missing (or both user/pwd)
    if (pwd is None or pwd == "") and DEFAULT_CRED_PATH.exists():
        try:
            lines = DEFAULT_CRED_PATH.read_text(encoding="utf-8").splitlines()
            file_user = lines[0].strip() if len(lines) > 0 else None
            file_pwd  = lines[1].strip() if len(lines) > 1 else None
            if not user and file_user:
                user = file_user
            if not pwd and file_pwd:
                pwd = file_pwd
        except Exception:
            pass  # fall through to whatever we have
...
def main():
    ...
    # Remove required positional hostname
    for action in list(parser._actions):
        if action.dest == "hostname":
            action.required = False
            action.nargs = "?"

    # Let it run with no flags at all.
    parser.set_defaults(hostname=DEFAULT_HOST, username=DEFAULT_USER, password=DEFAULT_PASS)
    ...
    host, user, pwd = resolve_connection_params(options)

    robot = sdk.create_robot(host)

    # ✅ Programmatic auth if we have both user & password
    if user and pwd:
        robot.authenticate(user, pwd)
    # ✅ If we only have a username, let the helper prompt for password
    elif user:
        bosdyn.client.util.authenticate(robot, user)
    # ✅ Fallback: prompt for both
    else:
        bosdyn.client.util.authenticate(robot)

    estop_client = robot.ensure_client(EstopClient.default_service_name)
    return build_and_run_app(host, estop_client, options)
