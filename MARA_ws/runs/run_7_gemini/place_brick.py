...
def lay_brick(x: float, y: float, z: float):
    ...
    return [
        ...
        {"sleep": 0.5},  # Longer settle time (was 0.2)
        "open",
        move(x - .02, y, z + 0.05, time=1.0), # Slower retract (was 0.5)
        move(x - .02, y, 0.30,     time=1.0), # Slower retract (was 0.5)
    ]
...
def _mobility_params():
    """Mobility params with yaw assist enabled for precision."""
    body_assist = spot_command_pb2.BodyControlParams.BodyAssistForManipulation(
        ...
        enable_body_yaw_assist=False,  # *** IMPROVEMENT: Enable yaw assist ***
    )
...
def run(robot, target: Tuple[float, float, float]):
    ...
    # (This one can be slightly stale, it's just for walking)
    ...
    time.sleep(1.0)
    # -----------------------------------------------------------------
    # --- START CRASH FIX & PRECISION IMPROVEMENT ---
    #
    # Get the fiducial transform ONCE, now that the base is stable.
    # All arm moves will be relative to this single, stable snapshot
    # to eliminate step-to-step jitter.
    print(f"Base is in position. Getting stable fiducial transform...")
    try:
        # Use a longer wait to ensure a good lock.
    ...
    # Determine the STABLE transform and frame name to use for arm commands
    if SEND_IN_ODOM:
        rs = robot_state_client.get_robot_state()
        tf = rs.kinematic_state.transforms_snapshot
        odom_T_vision = get_a_tform_b(tf, ODOM_FRAME_NAME, VISION_FRAME_NAME)
    ...
    print(f"Locked on {fid_name} in {frame_for_arm_cmds}. Starting arm sequence.")
    #
    # --- END CRASH FIX & PRECISION IMPROVEMENT ---
    # -----------------------------------------------------------------
    ...
    for i, step in enumerate(steps, start=1):
        ...
        # -----------------------------------------------------------------
        # --- START CRASH FIX (CONTINUED) ---
        #
        # We NO LONGER re-find the fiducial here.
        
        dx, dy, dz, pitch_deg, grip_frac, step_seconds = _split_step(step)
        rot = _quat_from_pitch_deg(pitch_deg)
        
        # This is the desired pose *relative to the fiducial*
        offset_fid = SE3(dx, dy, dz, rot)

        # --- Compute target pose in chosen frame ---
        # Calculate the target pose in the command frame (vision or odom)
        # using the STABLE transform we captured ONCE.
        target_pose_in_cmd_frame = stable_cmd_T_fid * offset_fid
        
        #
        # --- END CRASH FIX (CONTINUED) ---
        # -----------------------------------------------------------------
        ...
        print(
            ...
            f"in {frame_for_arm_cmds} pitch={pitch_deg:.1f}°, grip={grip_label}, seconds={secs_used:.2f}, "
            f"[body_follow={'ON' if USE_BODY_FOLLOW else 'OFF'}, "
            f"hip_assist={'ON' if ENABLE_HIP_HEIGHT_ASSIST else 'OFF'}]"
        )

        # Send the pose in the STABLE command frame (vision or odom)
